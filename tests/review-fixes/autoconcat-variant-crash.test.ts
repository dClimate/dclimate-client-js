...
const gatewayUrl = "https://autoconcat-variant-crash.test";
...
describe("loadDataset auto-concatenation", () => {
  beforeEach(() => {
  ...
  it("concatenates priority-tagged variants when no preferred fallback variant exists", async () => {
    ...
    const loadPromise = client.loadDataset({
      request: {
    ...
    const [result, metadata] = await loadPromise;

    expect(result).toBeInstanceOf(GeoTemporalDataset);
    expect((result as GeoTemporalDataset).coords.time).toEqual([
